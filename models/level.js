...
exports.getLevelByNumber = (number) => {
    const baseLevel = levels.find(l => l.number === number);
    const content = levelContent[number];

    if (!baseLevel || !content) {
        return null; // Return null if level is not found
    }

    // Combine base info and content
    return { ...baseLevel, ...content };
};